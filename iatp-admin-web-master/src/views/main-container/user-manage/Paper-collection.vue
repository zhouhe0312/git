<template>
    <div>
        <el-row type="flex" justify="space-between" class="search-form-row">
            <el-col :span="2">
                <div class="left_btn">
                    <el-button size="medium" v-if="handleButtonHash('user_management:paper_collect:delete')" style='background:#f38916;color: #fff;' @click='deleteWQ'>删除</el-button>
                </div>
            </el-col>
            <el-col :span="22">
                <el-form :inline="true" :model="searchInfo" class="wq-form-inline">
                    <el-form-item label="用户账户">
                        <el-input v-model="searchInfo.params.userName" clearable placeholder="请输入用户账户" class="search_right" ></el-input>
                    </el-form-item>
                ...
                <el-table
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <i @click="deleteWQ(scope.row)" class="el-icon-delete mybtn" style="color:#999;font-size:17px;cursor:pointer"  v-if="handleButtonHash('user_management:paper_collect:delete')"></i>

                            <!-- <el-link  v-if="handleButtonHash('user_management:paper_collect:delete')" type="info" :underline="false" @click="deleteWQ(scope.row)">删除</el-link> -->
                        </template>
                    </el-table-column>
                </el-table>
            </el-col>
...
//		    获取表格数据
            getData(value) {
                this.loading = true
                this.$store.dispatch('user/obtainUserCollectList',value)
                    .then(() => {
                        this.tableData  = this.$store.state.user.userCollectList
                        this.pages = this.$store.state.user.userCollectPage;
                        this.loading = false
                    })
                    .catch(() => {
                        this.loading = false
            ...
            deleteWQ(row) {
                ...
                that.deleteObj.del = {
                    async del() {
                        ...
                        that.$store.dispatch('user/deleteUserCollectAct',ids)
                            .then(() => {
