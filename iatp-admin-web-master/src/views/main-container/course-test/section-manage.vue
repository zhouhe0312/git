<template>
	<div class="mainPart">
		<el-row>
			<el-col :span="24">
				<div class="left_btn">
  					<el-button v-if="handleButtonHash('course_experiment:course:chapter:add')"  size="small" type="warning" style='background: #f38916;' @click='showAdd("addSectioneForm")'>新增章节</el-button>
  					<el-button v-if="handleButtonHash('course_experiment:course:chapter:edit')"  size="small" style='border:1px solid #f38916;color: #f38916;' @click='editTable("addSectioneForm")'>编辑章节</el-button>
                    <el-button v-if="handleButtonHash('course_experiment:course:chapter:delete')"  size="small" style='border:1px solid #f38916;color: #f38916;' @click='delTable'>删除章节</el-button>
        ...
        <el-row v-if="!getTreeList || getTreeList.length == 0" style='margin-top: 22px;'>
            <el-col>
                 <div style="margin:15% 0px 0px 42%">
                    ...
                    <p style="color:#AFAFAF;font-size:14px;margin-left: -50px;">您还没有添加章节哦，请点击<span style="color:#6A8BD1"> 新增章节 </span>按钮...</p>
                </div>
            </el-col>
        </el-row>
			...
			<el-col :span="20"  v-if="currNodeData.pid !== 0 && allNode.level == 2 "> <!--非父节点-->
                ...
                <div style=" margin:0px 0px 80px 20px">
                    <div style="float:left" >
                        <P  style="font-weight:700;font-size:14px">{{allNode.parent.label?allNode.parent.label:'根章节'}}</P>
                        <P  style="color:#999;font-size:13px;margin-top:10px">{{allNode.label?allNode.label:"暂无章节名称"}}</P> 
                    </div>
                     <div style="float:right" >
                        <el-button v-if="handleButtonHash('course_experiment:course:chapter:resource:add') && currNodeData.pid !=='0'" type="text" style="color:#f38916;font-size:14px" @click="uploadFile('uploadFileForm')">
                ...
                <div v-if="allNode.level == 2 && currNodeData.pid !== 0 && tableData.length == 0" style="margin:15% 0px 0px 42%">
                    <img src="@/assets/images/tip1.png">
                    <p style="color:#AFAFAF;font-size:14px;margin-left: -50px;">您还没有上传内容哦，请点击<span style="color:#6A8BD1"> 上传文件 </span>按钮...</p>
                </div>
                ...
                <draggable v-if="allNode.level == 2 && currNodeData.pid !== 0 && tableData"  v-model="tableData" :move="_getdata" @update="datadragEnd">
                    <el-card  v-for="(item, index) in tableData" :key="index" class="cardList">
                        <img v-if="item.fileExtensions == 'pdf'" src="@/assets/images/file_pdf.jpg" style="width:32px;height:40px;margin: -9px -5px;" class="filePic"/>
                        <img v-if="item.fileExtensions == 'pptx' || item.fileExtensions == 'ppt'" src="@/assets/images/file_ppt.jpg" style="width:32px;height:40px;margin: -9px -5px;" class="filePic"/>
                        <img v-if="item.fileExtensions !== 'pdf' && item.fileExtensions !== 'pptx' && item.fileExtensions !== 'ppt'" src="@/assets/images/file_video.jpg" style="width:32px;height:40px;margin: -9px -5px;" class="filePic"/>
                        <p class="fileName">{{item.resourceName}}</p>
                        <p class="fileSize">{{item.fileExtensions == 'pptx' || item.fileExtensions == 'ppt' || item.fileExtensions == 'pdf'?"文件页数：":"视频秒数："}}{{item.resourceSize?item.resourceSize+(item.fileExtensions == 'pptx' || item.fileExtensions == 'ppt' || item.fileExtensions == 'pdf'?"页":"秒"):`未知`}}</p>
                        <span style="float:right">
                            <span style=" font-size:12px; margin-right:45px;color:#999">{{item.createTime}}</span>
                            <i  v-if="handleButtonHash('course_experiment:course:chapter:resource:edit')" style="color:#999;font-size:17px;cursor:pointer;margin-right:10px" @click="editFile(item,'uploadFileForm')" class="el-icon-edit-outline mybtn"></i>
                            <i  v-if="handleButtonHash('course_experiment:course:chapter:resource:delete')" style="color:#999;font-size:17px;cursor:pointer" @click="delFile(item)" class="el-icon-delete mybtn" ></i>
                        </span>
                    </el-card>
                </draggable>
		...
		<!-- 新增/修改章节 -->
		<el-dialog :close-on-click-modal="false" width="40%" :title="sectionTitle" :visible.sync="sectionVisible"  :before-close="handleClose">
		  <el-form :model="addSectioneForm" ref="addSectioneForm" class="" label-width="90px" :rules="addSectioneFormRules">
            <div style="margin-left:-40px;padding-left:22px;">
                 <el-row>
                        <el-col :span="24">
                             <el-form-item label="所属章节" prop="pid">
                                <el-select popper-class="select-section" :disabled="isFather" style="width: 100%;"  placeholder="请选择所属章节" v-model="addSectioneForm.pid">
                                    <el-option v-for="item in getTreeFatList" :label="item.label" :key="item.id" :value="item.id">
                                        <span style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;" :title="item.label">{{ item.label }}</span>
                                    </el-option>
                                </el-select>
		...
		<!-- 新增/修章节下的文件 -->
		<el-dialog :close-on-click-modal="false" width="40%" :title="uploadFileTitle" :visible.sync="uploadFileVisible"  :before-close="handleClose">
		  <el-form :model="uploadFileForm" ref="uploadFileForm" class="" label-width="90px" :rules="uploadFileFormRules">
            <div style="margin-left:-40px;padding-left:22px;">
                <el-row>
                    <el-row>
                        <el-col :span="24">
                            <el-form-item class="_type" label="内容类型" prop="resourceType">
                                <el-radio-group v-model="uploadFileForm.resourceType" class="type" @change="handleUploadType">
       ...
       onblur="value=value.replace(/[^\d]/g,'')" v-model="uploadFileForm.resourceSize" ></el-input>
                                </el-form-item>
                        </el-col>
                </el-row>
...
<script>
	...
	import {addSectioneFormRules,uploadFileFormRules} from '@/util/rules'
	...
	export default {
		...
		mounted() {
            ...
            const obj = localStorage.getItem('JUMPCOURSEDATA')
		...
		methods:{
		   ...
		   // 获取表格数据
			...
			getTree(val){
                ...
                .then(() => {
                    this.getTreeList  = this.$store.state.courseSection.treeData[0].children;
                    let arr = JSON.parse(JSON.stringify(this.$store.state.courseSection.treeData[0].children))
                    if(arr !== null){
                         arr.unshift({
                            id:'0',
                            label:"根章节",
                        ...
                        this.getTreeFatList = arr
                    }else{
                       arr = JSON.parse(JSON.stringify(this.$store.state.courseSection.treeData)) || []
                       this.getTreeFatList = arr
                   }
					//console.log(this.getTreeFatList,'--')
                })
                .catch(() => {
            ...
            editFile(item,formName){
                ...
                setTimeout(()=>{
                   ...
                   this.uploadFileForm= {
                        ...
                        resourceSize: item.resourceSize?item.resourceSize:'',//文件：视频秒，文件页数
                        resourceType: item.resourceType?item.resourceType:2,//文件类型，1：视频，2：ppt，3：pdf
...
<style >
    ...
    ._upload-demo .el-upload-list__item.is-success:active,._upload-demo .el-upload-list__item.is-success.focusing{
        outline-width: 0 !important
    } 
    .mybtn:hover{
        color: #f38916 !important
    }
    .courseTree .el-tree-node__content{
        padding:3px 0px
    }
    .courseTree .el-tree-node.is-expanded .el-tree-node__children .custom-tree-node{
        padding-left: 18px !important;
        font-size:14px !important
    }
    .courseTree .custom-tree-node {
        overflow: hidden;
        text-overflow: ellipsis;
        white-space: nowrap;
    }
      /* .courseTree .el-tree-node__label {
        overflow: hidden;
        text-overflow: ellipsis;
        white-space: nowrap;
    } */
    ._upload-demo .el-upload--text{
        display: inline-block
    }
    ._upload-demo .el-upload-list{
        display: inline-block;
        padding-left: 92px;
        position: absolute;
        top: 36px
   ...
   .select-section .el-select-dropdown__list {
       max-width: 500px;
       min-width: 300px;
   }
    .fileName{
